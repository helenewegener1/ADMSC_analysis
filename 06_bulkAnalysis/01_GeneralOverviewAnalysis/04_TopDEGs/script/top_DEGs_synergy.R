...
# resLFC_synergy <- readRDS('06_bulkAnalysis/01_GeneralOverviewAnalysis/03_DifferentialExpression/out/resLFC_synergy.rds')

# filter top 50 up and down regulated genes 
res_filtered <- list()
resLFC_filtered <- list()

for (name in names(res_synergy)){
  
  res_tmp <- res_synergy[[name]]
  ...
  # resLFC_tmp <- resLFC_synergy[[name]]
...
# Save output
# Path to output file
out_file <- "06_bulkAnalysis/01_GeneralOverviewAnalysis/04_TopDEGs/out/res_synergy_filtered.xlsx"
...
meta_data <- dds_synergy@colData

for (comparison in names(res_synergy)){
  
  # comparison <- names(res_synergy)[4]
  
  res_tmp <- res_synergy[[comparison]]
  ...
  vsd <- vst(dds_synergy, blind = TRUE)
  vsd_mat <- assay(vsd)
  ...
  IDs <- str_split(comparison, '\\.') %>% unlist()
  # How ever long IDs is, only keep rows where colnames
  samples_of_condition <- meta_data %>% as.data.frame()
  if (length(IDs) == 2){
    samples_of_condition <- samples_of_condition %>% filter(!!sym(IDs[1]) == 1 & !!sym(IDs[2]) == 1)
  } else if (length(IDs) == 3) {
    samples_of_condition <- samples_of_condition %>% filter(!!sym(IDs[1]) == 1 & !!sym(IDs[2]) == 1 & !!sym(IDs[3]) == 1)
  }
  ...
  pdf(glue('06_bulkAnalysis/01_GeneralOverviewAnalysis/04_TopDEGs/plot/synergy_top_50_DEGs_heatmap_{comparison}.pdf'))
