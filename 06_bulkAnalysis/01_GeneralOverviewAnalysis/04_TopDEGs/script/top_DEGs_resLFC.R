...
library(rJava)
library(xlsx)
for (name in names(resLFC_filtered)){
...
for (comparison in names(resLFC)){
  ...
  pdf(glue('06_bulkAnalysis/01_GeneralOverviewAnalysis/04_TopDEGs/plot/LFCshrinked_top_50_DEGs_heatmap_{comparison}.pdf'))
  ...
  p <- pheatmap(mat_top_subset,
                ...
                show_colnames = TRUE,
                fontsize_row = 6,
                main = glue("Top 50 DEGs (LFC-shrinked) in {comparison}"))
  ...
  df_volcano %>% 
    ...
    theme_bw() +
    labs(
      title = glue("Volcano Plot (LFC-shrinked): {comparison}"),
...
deg_summary %>% 
  ...
  labs(
    title = "Number of DEGs per Comparison (LFC-shrinked)",
